(function (factory) {
	...
	var template = '<div id="'+defID+'{0}" class="dialog"><h2 style="color:black; font-family:\'SF Pro Text\'; padding-left:1%; padding-right:1%;">{1}</h2><p class="text-pad" style="color:gray;">{2}</p><div class="dialog-btn-div"><input type="submit" value="CANCEL" onclick="$({3}).hide(\'fast\', function(){ window[\'{4}\'](); });" class="dialog-btn-left" style="font-family:\'SF Pro Text\';"><input type="submit" onclick="$({5}).hide(\'fast\', function(){ window[\'{6}\'](); });" value="OK" class="dialog-btn-right"></div></div>';
	var template2 = '<div id="'+defID+'{0}" class="dialog"><h2 style="color:black; font-family:\'SF Pro Text\'; padding-left:1%; padding-right:1%;">{1}</h2><p class="text-pad" style="color:gray;">{2}</p><div class="dialog-btn-div"><input type="submit"onclick="$({3}).hide(\'fast\', function(){ window[\'{4}\'](); });" value="OK" class="dialog-btn-middle"></div></div>';
	var template3 = '<div id="'+defID+'{0}" class="dialog"><h2 style="color:black; font-family:\'SF Pro Text\'; padding-left:1%; padding-right:1%;">{1}</h2><p class="text-pad" style="color:gray;">{2}</p><input id="sd{3}_text" class="dialog-input" type="text" placeholer="{4}" value="{5}"><br><br><div class="dialog-btn-div"><input type="submit" value="CANCEL" onclick="$({6}).hide(\'fast\', function(){ window[\'{7}\'](); });" class="dialog-btn-left" style="font-family:\'SF Pro Text\';"><input type="submit" onclick="$({8}).hide(\'fast\', function(){ var text = $(\'#sd{9}_text\').val(); window[\'{10}\'](text); });" value="OK" class="dialog-btn-right"></div></div>';
	...
	function CreateRMenu(html, content){
		...
		var context_func = function(e){
			...
			$.each(content, function (index, value) {
				...
				app.on("mouseover", function(){
					...
					menu.add(menu2).on('mouseleave', function() {
						...
						setTimeout(function () {
							if (co)
								menu2.hide(100, function() { menu2.remove(); });
						}, 50);
					});
				});
			});
			...
			var func_remove = function(){
				menu.hide(100, function(){ menu.remove(); });
			}
